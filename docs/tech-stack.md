...
- **Playlist generation** (`generate-playlist`) — Builds a playlist recipe from 3–5 seed tracks using the hybrid Claude + math scoring pipeline. Triggered by `playlist/generate.requested` after the `playlist.generate` tRPC mutation inserts a `GENERATING` placeholder row; the function loads seeds, gets a Claude target + vibe name/description, computes a math centroid target, scores the user's library via the shared `scoreLibrary` helper, ranks/caps/truncates/shuffles, and flips the row to `PENDING`. Concurrency keyed on `playlistId` (limit 1) so retries can't race. `onFailure` marks the row `FAILED`. No Spotify push happens here — that waits for the user to click Save. See `docs/plans/completed/playlist-generation-hybrid.md`.
- **Playlist regenerate** (`regenerate-playlist`) — Re-rolls an existing `PENDING` or `SAVED` playlist against its stored recipe. Triggered by `playlist/regenerate.requested` after the `playlist.regenerate` tRPC mutation flips the row to `GENERATING` and threads `priorStatus` through the event. Reuses `scoreLibrary` + `computePerArtistCap` from the generate path, replaces `generatedTrackIds` via `updateTracks`, and — if `priorStatus === "SAVED"` — replaces the live Spotify playlist tracks via `replacePlaylistTracks`. Restores the prior status at the end. Seeds remain required tracks on every re-roll. Same concurrency key (`playlistId`, limit 1) as generate so two tabs regenerating the same playlist serialize.
- **Playlist top-up** (`top-up-playlist`) — Appends new matches to an existing `PENDING` or `SAVED` playlist without touching existing order. Triggered by `playlist/top-up.requested`. Pre-populates `rankAndFilter`'s `initialArtistCounts` from existing tracks so the per-artist cap applies across existing + new, uses `excludeIds` to drop already-present tracks, and walks until the top-up budget (25% of the original target, 10-minute floor) is hit. Appends via `appendTracks` on the DB row and — if `SAVED` — via `addTracksToPlaylist` on Spotify (new URIs only).
